...
export async function addActivityMediaAction(activityId: string, formData: FormData): Promise<AddResult> {
  const supabase = await createClient();
  const files = formData.getAll('files').filter((f): f is File => f instanceof File && f.size > 0);
  if (files.length === 0) return {ok: false, message: 'Aucun fichier sélectionné.'};

  const {count} = await supabase.from('activity_media').select('*', {count: 'exact', head: true}).eq('activity_id', activityId);
  let order = count ?? 0;
  ...
  for (const file of files) {
    ...
    const {data: row, error: insertError} = await supabase
      .from('activity_media')
      .insert({activity_id: activityId, media_url: pub.publicUrl, media_type, display_order: order++})
...
export async function deleteActivityMediaAction(mediaId: string): Promise<DeleteResult> {
  const supabase = await createClient();
  const {error} = await supabase.from('activity_media').delete().eq('id', mediaId);
